const express = require("express");
const cors = require("cors");
const path = require("path");
const authRouter = require("./routes/auth");

const app = express();
const PORT = process.env.PORT || 3000;

app.use(cors());
app.use(express.json());

// Serve static website files from root
app.use(express.static(path.join(__dirname, "..")));

app.use("/api/auth", authRouter);

// Download extension endpoint
app.get("/api/download-extension", (req, res) => {
  const zipPath = path.join(__dirname, "..", "public", "CleanAdsSurf-Extension.zip");
  
  res.download(zipPath, "CleanAdsSurf-Extension.zip", (err) => {
    if (err) {
      console.error("Error downloading extension:", err);
      res.status(404).json({ error: "Extension ZIP not found" });
    }
  });
});

app.get("/health", (req, res) => {
  res.json({ status: "ok", message: "CleanAdsSurf API is running" });
});

app.listen(PORT, () => {
  console.log(`\nCleanAdsSurf API server running at http://localhost:${PORT}`);
  console.log(`Available endpoints:`);
  console.log(`   GET    /                          (Website)`);
  console.log(`   POST   /api/auth/signup`);
  console.log(`   POST   /api/auth/login`);
  console.log(`   GET    /api/auth/me`);
  console.log(`   GET    /api/download-extension    (Download extension ZIP)`);
  console.log(`   GET    /health`);
  console.log(`\nDummy credentials for testing:`);
  console.log(`   Username: admin`);
  console.log(`   Password: admin\n`);
});
